...
- **`sabnzbd_api.py`** — `APIRouter` (mounted at `/sabnzbd` in `main.py`) implementing just enough of the SABnzbd HTTP API for Sonarr's download client integration: `version`, `get_cats`, `get_config`, `fullstatus`, `addurl`, `queue`, `history` (plus `queue`/`history` `delete`). `get_config`'s `misc.complete_dir` must be an absolute path (Sonarr's `GetCategories` falls back to a `fullstatus` call otherwise) and its `categories[].dir` must not end in `*` (Sonarr reads that as "job folders disabled" and raises a validation warning). `JobManager` holds an in-memory `nzo_id -> Job` dict and a background `asyncio.Task` per job that calls `tvdb_client.resolve_episode` then `dropout_downloader.download`. `addurl`'s `name` param is one of *our own* release links (see below) — `parse_nzb_link` pulls tvdbid/season/episode back out of it; nothing external ever dereferences that URL. `job_manager` is a module-level singleton set by `init_job_manager()` in `main.py`'s `lifespan` — same pattern as `main.py`'s own globals, see below.
