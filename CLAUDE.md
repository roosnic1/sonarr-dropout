...
- **`dropout_downloader.py`** — thin wrapper around `yt-dlp`'s Python API. `download(url, dest_dir, netrc_path)` runs the blocking yt-dlp call via `asyncio.to_thread` and returns `dest_dir` (not a filename — SABnzbd's `history` API reports a folder, and Sonarr's importer scans it). yt-dlp's dropout.tv extractor looks up credentials under netrc machine name `dropout` specifically.
- **`sabnzbd_api.py`** — `APIRouter` (mounted at `/sabnzbd` in `main.py`) implementing just enough of the SABnzbd HTTP API for Sonarr's download client integration: `version`, `get_cats`, `fullstatus`, `addurl`, `queue`, `history` (plus `queue`/`history` `delete`). `JobManager` holds an in-memory `nzo_id -> Job` dict and a background `asyncio.Task` per job that calls `tvdb_client.resolve_episode` then `dropout_downloader.download`. `addurl`'s `name` param is one of *our own* release links (see below) — `parse_nzb_link` pulls tvdbid/season/episode back out of it; nothing external ever dereferences that URL. `job_manager` is a module-level singleton set by `init_job_manager()` in `main.py`'s `lifespan` — same pattern as `main.py`'s own globals, see below.
...
- **`main.py`** — FastAPI app and all HTTP routes. Key things to know before touching it:
  ...
  - Each `ReleaseItem.link` is built as `{public_url}/sabnzbd/nzb/{tvdbid}/{season}/{episode}` — this URL is never actually fetched by anything; it's only ever handed back to `sabnzbd_api`'s `addurl` handler by Sonarr, which parses it. Keep the two encodings (`main.py` building it, `sabnzbd_api.parse_nzb_link` reading it) in sync if the format changes.
